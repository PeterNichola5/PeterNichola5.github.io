...
const tan = Math.tan(7*Math.PI/4);
const sine = Math.sin(7*Math.PI/4);
const cos = Math.cos(7*Math.PI/4);

const z = tan * (viewX/2);
const ah = (viewY/2 - z);
const a = sine * ah;
const hiddenGradientLineLength = (viewX/2) / cos;
...
const updateTitles = () => {
    for(const title of titles) {
        ...
        const gStart = (gradientLineLength/2) + ((elLoc.top + window.screenY) - elLoc.left);
        const gEnd = (gradientLineLength/2) + ((elLoc.bottom + window.screenY) - elLoc.right);
        const colorVals = [
